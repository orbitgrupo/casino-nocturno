(function(){
  ...
  if(game==='domino'){for(const href of ['css/domino-teams.css','css/domino-numbered-lineup.css']){const teamCss=document.createElement('link');teamCss.rel='stylesheet';teamCss.href=href;document.head.appendChild(teamCss)}}
  ...
  function dominoTeamsMarkup(host){if(room.game_type!=='domino')return'';const players=members.filter(member=>member.member_role!=='spectator'&&member.seat!==null).sort((a,b)=>(a.join_number||99)-(b.join_number||99)),lineup=Array.isArray(room.domino_lineup)?room.domino_lineup:[],locked=['playing','round-over','match-over'].includes(room.game_state?.phase),name=value=>{if(String(value).toUpperCase()==='BOT')return'Bot';const player=players.find(p=>Number(p.join_number)===Number(value));return player?`#${player.join_number} ${escape(player.display_name)}`:`Lugar ${escape(value||'vacío')}`};if(!host&&lineup.length===4)return`<div class="domino-pair-summary"><span><b>PAREJA A</b>${name(lineup[0])} + ${name(lineup[2])}</span><span><b>PAREJA B</b>${name(lineup[1])} + ${name(lineup[3])}</span></div>`;if(!host)return'';const defaults=lineup.length===4?lineup:(players.length===2?['1','2','BOT','BOT']:['1','2','3','4']);return `<section class="domino-team-builder numeric-builder"><header><div><small>PAREJAS DE DOMINÓ</small><b>Escribe el número del jugador o BOT en cada lugar</b></div></header><div class="numbered-player-list">${players.map(p=>`<span><b>#${p.join_number||p.seat+1}</b>${escape(p.display_name)}</span>`).join('')}</div><div class="domino-table-slots">${defaults.map((value,index)=>`<label class="table-slot slot-${index+1}"><span>LUGAR ${index+1}<small>${index%2===0?'PAREJA A':'PAREJA B'}</small></span><input data-lineup-slot="${index}" maxlength="3" value="${escape(value)}" ${locked?'disabled':''}></label>`).join('')}</div><div class="team-builder-actions"><button type="button" class="outline-btn" data-auto-pairs ${![2,4].includes(players.length)||locked?'disabled':''}>ACOMODAR POR ORDEN</button><button type="button" class="primary-btn" data-save-pairs ${![2,4].includes(players.length)||locked?'disabled':''}>GUARDAR PAREJAS</button></div><p>${locked?'La formación está bloqueada porque la mano ya comenzó.':players.length===2?'Puedes completar los dos lugares restantes con BOT.':players.length===4?'Pareja A ocupa 1 y 3; Pareja B ocupa 2 y 4.':'Espera 2 o 4 jugadores para formar la mesa.'}</p></section>`}
  function setupDominoTeamBuilder(){const builder=$('.domino-team-builder');if(!builder)return;const players=members.filter(member=>member.member_role!=='spectator'&&member.seat!==null).sort((a,b)=>(a.join_number||99)-(b.join_number||99)),inputs=[...builder.querySelectorAll('[data-lineup-slot]')];builder.querySelector('[data-auto-pairs]')?.addEventListener('click',()=>{const values=players.length===2?[String(players[0].join_number),String(players[1].join_number),'BOT','BOT']:players.map(p=>String(p.join_number));inputs.forEach((input,index)=>input.value=values[index]||'')});builder.querySelector('[data-save-pairs]')?.addEventListener('click',()=>{const slots=inputs.map(input=>input.value.trim().toUpperCase());if(slots.some(value=>!value))return message('Completa los cuatro lugares.','error');run(()=>rpc('host_set_domino_lineup',{p_room_id:room.id,p_slots:slots}))})}
  function renderRoom(){
