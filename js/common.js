(function(global){'use strict';
 ...
 if(page==='index'){const profileLink=document.createElement('a');profileLink.href='profile.html';profileLink.textContent='MI PERFIL Y PUNTOS';profileLink.style.cssText='position:fixed;z-index:70;right:16px;bottom:16px;padding:10px 14px;border:1px solid rgba(232,189,104,.35);border-radius:10px;color:#e8bd68;background:rgba(5,23,17,.92);font:800 8px sans-serif;letter-spacing:.12em;text-decoration:none;box-shadow:0 10px 30px rgba(0,0,0,.35)';document.body.appendChild(profileLink)}
 if(page==='index'){const grid=document.querySelector('.game-grid');if(grid&&!grid.querySelector('[href="poker.html"]')){const tile=document.createElement('a');tile.className='game-tile poker-tile';tile.href='poker.html';tile.innerHTML='<div class="game-icon">♠</div><div><small>5 JUGADORES · TEXAS HOLD’EM</small><h2>Póker</h2><p>Ciegas, apuestas, cinco cartas comunitarias y enfrentamiento contra cuatro bots.</p><b>ENTRAR A LA MESA →</b></div>';grid.appendChild(tile)}}
 if(['blackjack','roulette','tres-y-dos','domino','poker'].includes(page)){
   ...
   load('js/supabase-config.js').then(()=>load('js/room-lobby.js')).then(()=>page==='domino'?load('js/domino-online.js'):page==='blackjack'?load('js/blackjack-online.js'):null).catch(()=>{});
 }
})(window);
