(function(){
  ...
  if(game==='domino'){const teamCss=document.createElement('link');teamCss.rel='stylesheet';teamCss.href='css/domino-teams.css';document.head.appendChild(teamCss)}
  ...
  function dominoTeamsMarkup(host){if(room.game_type!=='domino')return'';const players=members.filter(member=>member.seat!==null),locked=['playing','round-over','match-over'].includes(room.game_state?.phase);if(!host)return players.some(p=>p.team_no!==null&&p.team_no!==undefined)?`<div class="domino-pair-summary">${[0,1].map(team=>`<span><b>PAREJA ${team?'B':'A'}</b>${players.filter(p=>Number(p.team_no)===team).map(p=>escape(p.display_name)).join(' + ')||'Sin definir'}</span>`).join('')}</div>`:'';return `<section class="domino-team-builder"><header><div><small>PAREJAS DE DOMINÓ</small><b>Arrastra los jugadores para formar los equipos</b></div></header><div class="domino-team-zones"><div class="team-dropzone" data-team="waiting"><strong>SIN ASIGNAR</strong>${players.filter(p=>p.team_no===null||p.team_no===undefined).map(teamPlayerMarkup).join('')}</div><div class="team-dropzone team-a" data-team="0"><strong>PAREJA A</strong>${players.filter(p=>Number(p.team_no)===0).map(teamPlayerMarkup).join('')}</div><div class="team-dropzone team-b" data-team="1"><strong>PAREJA B</strong>${players.filter(p=>Number(p.team_no)===1).map(teamPlayerMarkup).join('')}</div></div><div class="team-builder-actions"><button type="button" class="outline-btn" data-auto-pairs ${players.length!==4||locked?'disabled':''}>FORMAR AUTOMÁTICAMENTE</button><button type="button" class="primary-btn" data-save-pairs ${players.length!==4||locked?'disabled':''}>GUARDAR PAREJAS</button></div><p>${locked?'Las parejas están bloqueadas porque la mano ya comenzó.':players.length===4?'Cada pareja ocupará asientos alternados.':'Se necesitan exactamente 4 jugadores para crear dos parejas.'}</p></section>`}
  function teamPlayerMarkup(member){return `<div class="team-player" draggable="true" data-team-player="${escape(member.user_id)}"><span>${escape(member.display_name.charAt(0).toUpperCase())}</span><b>${escape(member.display_name)}</b></div>`}
  function setupDominoTeamBuilder(){const builder=$('.domino-team-builder');if(!builder)return;let dragged='';builder.querySelectorAll('[data-team-player]').forEach(card=>card.addEventListener('dragstart',event=>{dragged=card.dataset.teamPlayer;event.dataTransfer.setData('text/plain',dragged);event.dataTransfer.effectAllowed='move'}));builder.querySelectorAll('[data-team]').forEach(zone=>{zone.addEventListener('dragover',event=>{event.preventDefault();zone.classList.add('drag-over')});zone.addEventListener('dragleave',()=>zone.classList.remove('drag-over'));zone.addEventListener('drop',event=>{event.preventDefault();zone.classList.remove('drag-over');const id=event.dataTransfer.getData('text/plain')||dragged,card=[...builder.querySelectorAll('[data-team-player]')].find(item=>item.dataset.teamPlayer===id);if(card)zone.append(card)})});builder.querySelector('[data-auto-pairs]')?.addEventListener('click',()=>{const cards=[...builder.querySelectorAll('[data-team-player]')],zones=[builder.querySelector('[data-team="0"]'),builder.querySelector('[data-team="1"]')];cards.forEach((card,index)=>zones[index%2].append(card))});builder.querySelector('[data-save-pairs]')?.addEventListener('click',()=>{const assignments=[0,1].flatMap(team=>[...builder.querySelectorAll(`[data-team="${team}"] [data-team-player]`)].map(card=>({user_id:card.dataset.teamPlayer,team_no:team})));if(assignments.filter(x=>x.team_no===0).length!==2||assignments.filter(x=>x.team_no===1).length!==2)return message('Cada pareja debe tener exactamente 2 jugadores.','error');run(()=>rpc('host_set_domino_teams',{p_room_id:room.id,p_assignments:assignments}))})}
  function renderRoom(){
    ...
    view.innerHTML=`<div class="room-code"><span>CÓDIGO DE INVITACIÓN</span><strong>${escape(room.invite_code)}</strong><button type="button" data-copy>Copiar invitación</button></div>
      ...
      <div class="online-members">${members.map(member=>`<article class="online-member ${member.user_id===room.host_id?'host':''}"><span class="online-avatar">${escape(member.display_name.charAt(0).toUpperCase())}</span><div><b>${escape(member.display_name)}</b><small>${member.user_id===room.host_id?'ANFITRIÓN · ':''}${member.seat===null?'MODERADOR':`ASIENTO ${member.seat+1}`}${room.game_type==='domino'&&member.team_no!==null&&member.team_no!==undefined?` · PAREJA ${Number(member.team_no)===0?'A':'B'}`:''}</small></div><strong>${member.credits.toLocaleString('es-ES')} puntos</strong>${host&&member.user_id!==user.id?`<button type="button" class="kick" data-kick="${member.user_id}">Expulsar</button>`:''}</article>`).join('')}</div>
