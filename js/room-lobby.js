(function(){
  ...
  function dominoTeamsMarkup(host){if(room.game_type!=='domino')return'';const players=members.filter(m=>m.member_role!=='spectator'&&m.seat!==null).sort((a,b)=>(a.join_number||99)-(b.join_number||99)),lineup=Array.isArray(room.domino_lineup)?room.domino_lineup:[],locked=['playing','round-over','match-over'].includes(room.game_state?.phase),label=value=>String(value).toUpperCase()==='BOT'?'BOT':`#${value}`,options=current=>`<option value="">VACÍO</option>${players.map(p=>`<option value="${p.join_number}" ${String(current)===String(p.join_number)?'selected':''}>#${p.join_number} · ${escape(p.display_name)}</option>`).join('')}${players.length<=2?`<option value="BOT" ${String(current).toUpperCase()==='BOT'?'selected':''}>BOT</option>`:''}`;if(!host&&lineup.length===4)return`<div class="domino-pair-summary"><span><b>PAREJA A</b>${label(lineup[0])} + ${label(lineup[2])}</span><span><b>PAREJA B</b>${label(lineup[1])} + ${label(lineup[3])}</span></div>`;if(!host)return'';const defaults=lineup.length===4?lineup:(players.length===2?[players[0]?.join_number,players[1]?.join_number,'BOT','BOT']:players.map(p=>p.join_number));return `<section class="domino-team-builder numeric-builder"><header><div><small>PAREJAS DE DOMINÓ</small><b>Selecciona quién ocupará cada lugar</b></div></header><div class="numbered-player-list">${players.map(p=>`<span><b>#${p.join_number}</b>${escape(p.display_name)}</span>`).join('')}</div><div class="domino-table-slots">${[0,1,2,3].map(index=>`<label class="table-slot"><span>LUGAR ${index+1}<small>${index%2===0?'PAREJA A':'PAREJA B'}</small></span><select data-lineup-slot="${index}" ${locked?'disabled':''}>${options(defaults[index]||'')}</select></label>`).join('')}</div><div class="team-builder-actions"><button type="button" class="outline-btn" data-auto-pairs ${![2,4].includes(players.length)||locked?'disabled':''}>ACOMODAR POR ORDEN</button><button type="button" class="primary-btn" data-save-pairs ${![2,4].includes(players.length)||locked?'disabled':''}>GUARDAR LUGARES</button></div><p>${locked?'La mano ya comenzó.':players.length===2?'Selecciona dos jugadores y dos BOT.':players.length===4?'Usa cada número una vez.':'Se necesitan 2 o 4 jugadores.'}</p></section>`}
