...
begin
  ...
  if exists(select 1 from public.casino_room_members m where m.room_id=v_room.id and m.profile_id=v_profile.id) then
    update public.casino_room_members m set user_id=v_user,credits=v_profile.points where m.room_id=v_room.id and m.profile_id=v_profile.id;
    return query select v_room.id,v_room.invite_code::text;return;
  end if;
