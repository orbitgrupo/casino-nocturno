(function(){
  ...
  const ROOM_KEYS=Object.keys(GAME_MAP).map(key=>`casino.onlineRoom.${GAME_MAP[key]}`);
  ...
  function clearSavedRooms(){ROOM_KEYS.forEach(key=>localStorage.removeItem(key))}
  ...
  async function joinRoom(form){const values=new FormData(form);const result=await rpc('join_casino_room_for_game',{p_invite_code:String(values.get('inviteCode')).trim().toUpperCase(),p_display_name:values.get('displayName'),p_game_type:game});await openRoom(result.room_id)}
  async function openRoom(id){room={id};await refresh();clearSavedRooms();localStorage.setItem(ROOM_KEY,room.id);subscribe();history.replaceState(null,'',`${location.pathname}?room=${room.invite_code}`)}
  ...
  function renderRoom(){
    ...
    view.innerHTML=`<div class="room-code"><span>CÓDIGO DE INVITACIÓN</span><strong>${escape(room.invite_code)}</strong><button type="button" data-copy>Copiar invitación</button></div>
      ...
      ${room.status==='active'?`<p class="sync-notice">${SYNC_ENGINE[room.game_type]?'La sala está activa y la partida se sincroniza en esta mesa.':'Esta sala funciona como antesala online. El motor sincronizado de este juego todavía no está conectado; si juegan ahora, cada dispositivo usa una partida local.'}</p>`:''}`;
