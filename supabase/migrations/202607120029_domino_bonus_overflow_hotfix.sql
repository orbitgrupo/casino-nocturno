-- Dominó online: descarta puntos no cerrables cuando sobrepasan o alcanzan la meta.
-- Aplica a:
-- - Pase inmediato (+25 no cerrables)
-- - Pase redondo / pase a todos (+25 no cerrables)
-- La capicúa y los puntos normales de dominó/tranca sí siguen pudiendo cerrar la partida.

create or replace function public.domino_try_add_nonclosing_bonus(p_state jsonb,p_seat integer)
returns jsonb
language plpgsql
immutable
...
begin
  ...
  if v_score+25>=v_target then
    return jsonb_build_object('state',v_out,'applied',false);
  end if;

  v_out:=jsonb_set(v_out,array['scores',v_key],to_jsonb(v_score+25),true);
...
begin
  ...
  if coalesce(v_state->>'pending_all_pass','')<>'' and (v_state->>'pending_all_pass')::int=p_seat then
    v_bonus:=public.domino_try_add_nonclosing_bonus(v_state,p_seat);
    ...
    if coalesce((v_bonus->>'applied')::boolean,false) then
      ...
      v_msg:=v_msg||' · Pase a todos descartado por sobrepaso';
    end if;
  end if;
...
begin
  ...
  if jsonb_typeof(v_pending)='object' and coalesce((v_pending->>'awarded')::boolean,false)=false and (v_pending->>'next')::int=p_seat then
    ...
    v_bonus:=public.domino_try_add_nonclosing_bonus(v_state,v_owner);
    v_state:=v_bonus->'state';
    ...
    if coalesce((v_bonus->>'applied')::boolean,false) then
      ...
      v_state:=jsonb_set(v_state,'{pending_single_pass_bonus}','null'::jsonb,true);
      v_msg:='Jugador pasa · Pase inmediato descartado por sobrepaso';
    end if;
...
revoke all on function public.domino_try_add_nonclosing_bonus(jsonb,integer),public.domino_apply_play(uuid,integer,text,text,boolean),public.domino_apply_pass(uuid,integer) from public,anon;
