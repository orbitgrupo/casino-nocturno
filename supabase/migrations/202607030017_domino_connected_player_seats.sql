-- El anfitrión asigna personas conectadas o BOT directamente a los cuatro lugares.
...
begin
  ...
  if not((v_humans=4 and v_bots=0) or (v_humans=2 and v_bots=2)) then raise exception 'USE_TWO_OR_FOUR_PEOPLE';end if;
  ...
  for v_slot in 0..3 loop
    v_value:=p_slots->>v_slot;
    if upper(v_value)<>'BOT' then update public.casino_room_members m set seat=v_slot,join_number=v_slot+1,team_no=v_slot%2,member_role=case when m.user_id=v_room.host_id then 'host' else 'player' end where m.room_id=p_room_id and m.user_id=v_value::uuid;end if;
  end loop;
